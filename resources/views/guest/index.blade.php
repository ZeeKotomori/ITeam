            ...
            <div class="bg-slate-700/30 text-text rounded-sm shadow-lg translate-y-40 duration-[2s] opacity-0" data-scroll data-scroll-class="showY">
                ...
                <div class="p-5">
                    ...
                    <div class="grid grid-cols-5 mt-2 gap-2">
                        <form class="col-span-2 p-2 rounded-md duration-300 bg-red-500 hover:bg-red-800" action="{{ route("user.like", $produk->id) }}" method="POST"  id="likesBtn" >
                            @csrf
                            <div class="flex">
                                <button data-id="{{$produk->id}}" type="submit" value="add" class="w-full flex gap-x-[30%]">
                                    <i data-feather="heart"></i>
                                    <p>{{ count($produk->likes) }}</p>
                                </button>
                            </div>
                        </form>
                        ...
                        <a class="col-span-5 p-2 rounded-md duration-300 bg-indigo-700 hover:bg-indigo-800" href="{{ route('user.links', $produk->id) }}"><button class="w-full flex justify-center gap-x-2"> Link <i data-feather="arrow-right"></i></button></a>
                    </div>
                </div>
            </div>
            @endforeach
